...
class VEXMixin(SimEngineBase):
    ...
    def _handle_vex_stmt(self, stmt: pyvex.stmt.IRStmt):
        ...
        if stmt.tag == 'Ist_NoOp':
            global Ist_NoOp_number
            Ist_NoOp_number = Ist_NoOp_number + 1
            global Ist_NoOp_time
            Ist_NoOp_time = Ist_NoOp_time + executed_time
        if stmt.tag == 'Ist_IMark':
            global Ist_IMark_number
            Ist_IMark_number = Ist_IMark_number + 1
            global Ist_IMark_time
            Ist_IMark_time = Ist_IMark_time + executed_time
        if stmt.tag == 'Ist_AbiHint':
            global Ist_AbiHint_number
            Ist_AbiHint_number = Ist_AbiHint_number + 1
            global Ist_AbiHint_time
            Ist_AbiHint_time = Ist_AbiHint_time + executed_time
        if stmt.tag == 'Ist_Put':
            global Ist_Put_number
            Ist_Put_number = Ist_Put_number + 1
            global Ist_Put_time
            Ist_Put_time = Ist_Put_time + executed_time
        if stmt.tag == 'Ist_PutI':
            global Ist_PutI_number
            Ist_PutI_number = Ist_PutI_number + 1
            global Ist_PutI_time
            Ist_PutI_time = Ist_PutI_time + executed_time
        if stmt.tag == 'Ist_WrTmp':
            global Ist_WrTmp_number
            Ist_WrTmp_number = Ist_WrTmp_number + 1
            global Ist_WrTmp_time
            Ist_WrTmp_time = Ist_WrTmp_time + executed_time
        if stmt.tag == 'Ist_Store':
            global Ist_Store_number
            Ist_Store_number = Ist_Store_number + 1
            global Ist_Store_time
            Ist_Store_time = Ist_Store_time + executed_time
        if stmt.tag == 'Ist_CAS':
            global Ist_CAS_number
            Ist_CAS_number = Ist_CAS_number + 1
            global Ist_CAS_time
            Ist_CAS_time = Ist_CAS_time + executed_time
        if stmt.tag == 'Ist_LLSC':
            global Ist_LLSC_number
            Ist_LLSC_number = Ist_LLSC_number + 1
            global Ist_LLSC_time
            Ist_LLSC_time = Ist_LLSC_time + executed_time
        if stmt.tag == 'Ist_MBE':
            global Ist_MBE_number
            Ist_MBE_number = Ist_MBE_number + 1
            global Ist_MBE_time
            Ist_MBE_time = Ist_MBE_time + executed_time
        if stmt.tag == 'Ist_Dirty':
            global Ist_Dirty_number
            Ist_Dirty_number = Ist_Dirty_number + 1
            global Ist_Dirty_time
            Ist_Dirty_time = Ist_Dirty_time + executed_time
        if stmt.tag == 'Ist_Exit':
            global Ist_Exit_number
            Ist_Exit_number = Ist_Exit_number + 1
            global Ist_Exit_time
            Ist_Exit_time = Ist_Exit_time + executed_time
        if stmt.tag == 'Ist_LoadG':
            global Ist_LoadG_number
            Ist_LoadG_number = Ist_LoadG_number + 1
            global Ist_LoadG_time
            Ist_LoadG_time = Ist_LoadG_time + executed_time
        if stmt.tag == 'Ist_StoreG':
            global Ist_StoreG_number
            Ist_StoreG_number = Ist_StoreG_number + 1
            global Ist_StoreG_time
            Ist_StoreG_time = Ist_StoreG_time + executed_time
    ...
    def _handle_vex_expr(self, expr: pyvex.expr.IRExpr):
        ...
        executed_time = end - start
        stmt=expr
        if stmt.tag == 'Iex_Binder':
            global Iex_Binder_number
            Iex_Binder_number = Iex_Binder_number + 1
            global Iex_Binder_time
            Iex_Binder_time = Iex_Binder_time + executed_time
        if stmt.tag == 'Iex_VECRET':
            global Iex_VECRET_number
            Iex_VECRET_number = Iex_VECRET_number + 1
            global Iex_VECRET_time
            Iex_VECRET_time = Iex_VECRET_time + executed_time
        if stmt.tag == 'Iex_GSPTR':
            global Iex_GSPTR_number
            Iex_GSPTR_number = Iex_GSPTR_number + 1
            global Iex_GSPTR_time
            Iex_GSPTR_time = Iex_GSPTR_time + executed_time
        if stmt.tag == 'Iex_GetI':
            global Iex_GetI_number
            Iex_GetI_number = Iex_GetI_number + 1
            global Iex_GetI_time
            Iex_GetI_time = Iex_GetI_time + executed_time
        if stmt.tag == 'Iex_RdTmp':
            global Iex_RdTmp_number
            Iex_RdTmp_number = Iex_RdTmp_number + 1
            global Iex_RdTmp_time
            Iex_RdTmp_time = Iex_RdTmp_time + executed_time
        if stmt.tag == 'Iex_Get':
            global Iex_Get_number
            Iex_Get_number = Iex_Get_number + 1
            global Iex_Get_time
            Iex_Get_time = Iex_Get_time + executed_time
        if stmt.tag == 'Iex_Qop':
            global Iex_Qop_number
            Iex_Qop_number = Iex_Qop_number + 1
            global Iex_Qop_time
            Iex_Qop_time = Iex_Qop_time + executed_time
        if stmt.tag == 'Iex_Triop':
            global Iex_Triop_number
            Iex_Triop_number = Iex_Triop_number + 1
            global Iex_Triop_time
            Iex_Triop_time = Iex_Triop_time + executed_time
        if stmt.tag == 'Iex_Binop':
            global Iex_Binop_number
            Iex_Binop_number = Iex_Binop_number + 1
            global Iex_Binop_time
            Iex_Binop_time = Iex_Binop_time + executed_time
        if stmt.tag == 'Iex_Unop':
            global Iex_Unop_number
            Iex_Unop_number = Iex_Unop_number + 1
            global Iex_Unop_time
            Iex_Unop_time = Iex_Unop_time + executed_time
        if stmt.tag == 'Iex_Load':
            global Iex_Load_number
            Iex_Load_number = Iex_Load_number + 1
            global Iex_Load_time
            Iex_Load_time = Iex_Load_time + executed_time
        if stmt.tag == 'Iex_Const':
            global Iex_Const_number
            Iex_Const_number = Iex_Const_number + 1
            global Iex_Const_time
            Iex_Const_time = Iex_Const_time + executed_time
        if stmt.tag == 'Iex_ITE':
            global Iex_ITE_number
            Iex_ITE_number = Iex_ITE_number + 1
            global Iex_ITE_time
            Iex_ITE_time = Iex_ITE_time + executed_time
        if stmt.tag == 'Iex_CCall':
            global Iex_CCall_number
            Iex_CCall_number = Iex_CCall_number + 1
            global Iex_CCall_time
            Iex_CCall_time = Iex_CCall_time + executed_time
